  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50/40 to-white">
      ...
      <div className="bg-white border-b border-purple-100 px-10 md:px-20 lg:px-32 py-10">
        <div className="inline-flex items-center gap-2 bg-purpleimport React, { useEffect, useState } from 'react';
import AddResume from './components/AddResume';
import { useUser } from '@clerk/clerk-react';
import GlobalApi from "../../service/GlobalApi";
import ResumeCardItem from './components/ResumeCardItem';
import { Sparkles, FileText, Clock, FileX } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50/40 to-white">
      ...
      <div className="px-10 md:px-20 lg:px-32 py-10">
        {loading ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
            {Array.from({ length: 5 }).map((_, i) => (
              <div
                key={i}
                className="w-full aspect-[3/4] rounded-2xl bg-purple-100/50 animate-pulse"
              />
            ))}
          </div>
        ) : (
        ...
        {!loading && ResumeList.length === 0 && (
          <div className="flex flex-col items-center justify-center text-center mt-10 py-10">
...
export default DashBoard;-100 text-purple-700 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
          <Sparkles size={12} />
          AI Powered Resume Builder
        </div>
        <h1 className="text-4xl font-extrabold text-gray-900 tracking-tight">
